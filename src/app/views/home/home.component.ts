...
export class HomeComponent implements OnInit {
  @ViewChild(MatTable)  //Serve para visualizar um componente pelo id
  table!: MatTable<any>;
      ...
      if (result != undefined) {
        if (this.dataSource.map(p => p.id).includes(result.id)) {
          ...
          this.periodicElementService.createElements(result)
            .subscribe((data: PeriodicElement) => {
              this.dataSource.push(data); //Adicionando o resultado na tabela (dataSource)
              this.table.renderRows(); //Renderizando a tabela
            })
        }
      }
    });
